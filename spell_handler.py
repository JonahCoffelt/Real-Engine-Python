...
class Spell():
    ...
    def get_bullets(self, pos, direction : np.array):
        ...
        match self.launch_type:
            case 'straight': return [self.spell_handler.bullet_handler.add_bullet(Bullet(self, self.spell_handler.bullet_handler, pos + direction, self.spell_handler.launch_handler.get_straight(direction, self.speed))) for direction in directions]
            case 'lob': return [self.spell_handler.bullet_handler.add_bullet(Bullet(self, self.spell_handler.bullet_handler, pos + direction, self.spell_handler.launch_handler.get_lob(direction, self.speed, -9.8))) for direction in directions]
